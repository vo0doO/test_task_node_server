...
describe( "Интеграционные  тесты точки api платёжных транзакций", async () =>
{
    ...
    describe( 'GET /api/bills/items => ! "Массив транзакций"', async () =>
    {
        ...
        {
            for ( const v of Object.values( respBills[ 0 ] ) )
            {
                try
                {
    ...
    describe(
        'GET /api/bills/filteredByDate?dateFrom=[Date]&dateTo=[Date]=> \
            "Отфильтрованный по датам массив транзакций"',
        async () =>
        {
            beforeEach( async () =>
            {
                await req.withFilter.then( ( response ) =>
                {
